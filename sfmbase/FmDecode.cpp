...
PilotPhaseLock::PilotPhaseLock(double freq, double bandwidth, double minsignal_on, double minsignal_off)
{
    /*
    ...
    m_minsignal_on  = minsignal_on;
    m_minsignal_off = std::min(minsignal_on, minsignal_off);
                             ...
                             SampleVector& samples_out)
{
    ...
    if (m_lock_cnt >= m_lock_delay) {
        // Locked: keep lock until pilot drops below the (lower) OFF threshold.
        if (2 * m_pilot_level < m_minsignal_off) {
            m_lock_cnt = 0;
        }
    } else {
        // Unlocked: acquire lock only when pilot exceeds the ON threshold.
        if (2 * m_pilot_level > m_minsignal_on) {
            m_lock_cnt += n;
        } else {
            m_lock_cnt = 0;
        }
...
FmDecoder::FmDecoder(double sample_rate_if,
                     ...
                     double pilot_minsignal_on,
                     double pilot_minsignal_off,
                     bool force_stereo_output)
    ...
    , m_force_stereo_output(force_stereo_output)
    ...
    , m_pilotpll(pilot_freq / m_sample_rate_baseband,       // freq
                 ...
                 pilot_minsignal_on,
                 pilot_minsignal_off)                        // minsignal (was 0.04)
                        ...
                        SampleVector& audio)
{
    ...
    if (m_stereo_enabled)
    {
        ...
        const bool stereo_output = m_stereo_detected || m_force_stereo_output;
