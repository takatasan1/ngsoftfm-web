...
NGSoftFM requires:
 ...
 - Linux (primary)
 - Windows (native build is possible for file/stdout output; no ALSA)
 - C++11
...
<h2>All options</h2>
 ...
 - `-P [device]` Play audio via ALSA device (Linux only; default `default`). Use `aplay -L` to get the list of devices for your system
 - `-T filename` Write pulse-per-second timestamps. Use filename '-' to write to stdout
 - `-b seconds` Set audio buffer size in seconds

<h1>Windows (native, no WSL)</h1>

You can run NGSoftFM on Windows without WSL by building a native `softfm.exe` and using stdout (`-R -`) + `ffmpeg` for playback/HTTP streaming.

Notes:

 - ALSA output (`-P`) is Linux-only and is not available in Windows builds.
 - On Windows, only the backends with installed libraries are built; by default the Windows build is RTL-SDR-only.

<h2>1) Driver (RTL-SDR)</h2>

Install a WinUSB driver for your RTL-SDR dongle (commonly done with Zadig). This is required for `librtlsdr` on Windows.

<h2>2) Build (MSYS2/MinGW)</h2>

Install MSYS2 and open the "MSYS2 UCRT64" shell, then:
...
<h2>3) Stream on Windows</h2>

Install `ffmpeg` for Windows and ensure `ffmpeg.exe` is in PATH.

Then from PowerShell:

 - `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\Start-FMStream-Native.ps1 -Freq 80000000 -Gain 19.7 -PilotMin 0.01 -PilotHyst 0.6`

Open `http://localhost:8000/fm.mp3` in a browser or VLC.
