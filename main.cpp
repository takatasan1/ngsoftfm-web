...
#if defined(SOFTFM_HAS_RTLSDR)
#include "RtlSdrSource.h"
#endif
#if defined(SOFTFM_HAS_HACKRF)
#include "HackRFSource.h"
#endif
#if defined(SOFTFM_HAS_AIRSPY)
#include "AirspySource.h"
#endif
#if defined(SOFTFM_HAS_BLADERF)
                       ...
                       unsigned int buf_minfill)
{
    while (!stop_flag.load()) {
        ...
        if (!output->write(samples)) {
            std::string err = output->error();
            if (!err.empty()) {
                fprintf(stderr, "ERROR: AudioOutput: %s\n", err.c_str());
            } else {
                fprintf(stderr, "ERROR: AudioOutput: write failed\n");
            }

            // Most common cause is a broken pipe (e.g., streaming client disconnected).
            // Stop cleanly instead of spamming errors in a tight loop.
...
static void handle_sigterm(int sig)
{
    ...
    // Keep this handler simple and portable (MinGW does not provide sigaction/strsignal).
...
void usage()
{
    ...
    "Usage: softfm [options]\n"
            ...
            "  --force-stereo  Force stereo output even without pilot lock\n"
            "  -R filename    Write audio data as raw S16_LE samples\n"
            "                 use filename '-' to write to stdout\n"
            "  -W filename    Write audio data to .WAV file\n"
#if defined(SOFTFM_HAS_ALSA)
            "  -P [device]    Play audio via ALSA device (default 'default')\n"
#endif
            "  -T filename    Write pulse-per-second timestamps\n"
            "                 use filename '-' to write to stdout\n"
            "  -b seconds     Set audio buffer size in seconds\n"
            "  --pilot-min x      Stereo pilot lock threshold (default 0.01)\n"
            "  --pilot-min-off x  Stereo pilot unlock threshold (default 0.008)\n"
            "  --pilot-hyst x     Convenience: set pilot-min-off = pilot-min * x (0 < x <= 1)\n"
            "\n"
...
#else
    fprintf(stderr, "ERROR: RTL-SDR support not built in this binary\n");
    return false;
#endif
    }
...
#else
    fprintf(stderr, "ERROR: HackRF support not built in this binary\n");
    return false;
#endif
    }
...
#else
    fprintf(stderr, "ERROR: Airspy support not built in this binary\n");
    return false;
#endif
    }
...
#else
    fprintf(stderr, "ERROR: BladeRF support not built in this binary\n");
    return false;
#endif
    }
    else
...
int main(int argc, char **argv)
{
    ...
    int     pcmrate = 48000;
    bool    stereo  = true;
    bool    force_stereo_output = false;
    enum OutputMode {
        MODE_RAW,
        MODE_WAV,
#if defined(SOFTFM_HAS_ALSA)
    ...
    FILE *  ppsfile = NULL;
    double  bufsecs = -1;
    double  pilot_minsignal_on = 0.01;
    double  pilot_minsignal_off = 0.008;
    double  pilot_hyst = -1;
    bool    pilot_hyst_set = false;
    ...
    const struct option longopts[] = {
        ...
        { "pps",        1, NULL, 'T' },
        { "buffer",     1, NULL, 'b' },
        { "pilot-min",      1, NULL, OPT_PILOT_MIN },
        { "pilot-min-off",  1, NULL, OPT_PILOT_MIN_OFF },
        { "pilot-hyst",     1, NULL, OPT_PILOT_HYST },
        { "force-stereo",   0, NULL, OPT_FORCE_STEREO },
        { NULL,         0, NULL, 0 } };
    ...
    while ((c = getopt_long(argc, argv,
        ...
        switch (c) {
            ...
            case 'd':
                ...
                else if (!parse_int(optarg, devidx))
                {
                    badarg("-d");
                }
                break;
            ...
            case OPT_FORCE_STEREO:
                force_stereo_output = true;
                break;
            case 'R':
                outmode = MODE_RAW;
                filename = optarg;
            ...
            case 'b':
                ...
                break;
            case OPT_PILOT_MIN:
                if (!parse_dbl(optarg, pilot_minsignal_on) || pilot_minsignal_on <= 0) {
                    badarg("--pilot-min");
                }
                break;
            case OPT_PILOT_MIN_OFF:
                if (!parse_dbl(optarg, pilot_minsignal_off) || pilot_minsignal_off < 0) {
                    badarg("--pilot-min-off");
                }
                break;
            case OPT_PILOT_HYST:
                if (!parse_dbl(optarg, pilot_hyst) || pilot_hyst <= 0 || pilot_hyst > 1) {
                    badarg("--pilot-hyst");
                }
                pilot_hyst_set = true;
                break;
    ...
    if (pilot_hyst_set) {
        pilot_minsignal_off = pilot_minsignal_on * pilot_hyst;
    }

    // Clamp OFF threshold to <= ON threshold.
    if (pilot_minsignal_off > pilot_minsignal_on) {
        pilot_minsignal_off = pilot_minsignal_on;
    }

    if (!stereo && force_stereo_output)
    {
        usage();
        fprintf(stderr, "ERROR: --force-stereo can not be used with -M (stereo disabled)\n");
        exit(1);
    }
...
#if defined(_WIN32)
    // MinGW: use signal() (sigaction is not available).
    ...
    // Query/open SDR device early so "-d list" works without requiring an audio output.
    if (!get_device(devnames, devtype_str, &srcsdr, devidx))
    {
        // If the user explicitly asked for a device list, treat it as a successful command.
        exit(list_devices ? 0 : 1);
    }
...
#if !defined(SOFTFM_HAS_ALSA)
    // On platforms without live audio output, require explicit selection of WAV/RAW output.
    if (outmode == MODE_RAW && filename.empty())
    {
        usage();
        fprintf(stderr, "ERROR: No audio output selected. Use -R - (stdout) or -W <file>.\n");
        delete srcsdr;
        exit(1);
    }
#endif
    ...
    bool interactive_out = (outmode == MODE_RAW && filename == "-");
#if defined(SOFTFM_HAS_ALSA)
    interactive_out = interactive_out || (outmode == MODE_ALSA);
#endif

    if (bufsecs < 0 && interactive_out)
    {
